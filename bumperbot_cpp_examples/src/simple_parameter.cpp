...
class SimpleParameter : public rclcpp::Node {
   public:
    SimpleParameter() : Node("simple_parameter") {
        ...
        declare_parameter<std::string>("simple_string_param", "Berkay");
    ...
    rcl_interfaces::msg::SetParametersResult paramChangeCallback(
        ...
        for (const auto& param : parameters) {
            if (param.get_name() == "simple_int_param" &&
                ...
                RCLCPP_INFO_STREAM(get_logger(), "Param simple_int_param changed! New value is: "
                                                     << param.as_int());
            }

            if (param.get_name() == "simple_string_param" &&
                param.get_type() == rclcpp::ParameterType::PARAMETER_STRING) {
                RCLCPP_INFO_STREAM(get_logger(), "Param simple_string_param changed! New value is: "
                                                     << param.as_string());
            }
        }
